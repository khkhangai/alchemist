		...
		<tbody>
			<tr>
				<td>
					...
					{vtranslate($SINGLE_MODULE, $MODULE)} {vtranslate('LBL_FOUND')}.{if $IS_MODULE_EDITABLE} <a href="{$MODULE_MODEL->getCreateRecordUrl()}">{vtranslate($SINGLE_MODULE, $MODULE)}</a> {vtranslate('LBL_CREATE')}{/if}
				</td>
			</tr>
		</tbody>
	</table>
